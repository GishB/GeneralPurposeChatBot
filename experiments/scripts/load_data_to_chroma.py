...
def main():
    parser = argparse.ArgumentParser(description="Заполняем векторную базу данных исходя из MarkDown документов")
    parser.add_argument("--source-dir", type=Path, default=Path(__file__).parent.parent / "data")
    parser.add_argument(
        "--collection",
        type=str,
        default=os.getenv("COLLECTION_NAME", "PRODUCTION_PROFKOM"),
    )
    parser.add_argument("--chroma-host", type=str, default=os.getenv("CHROMA_HOST", "127.0.0.1"))
