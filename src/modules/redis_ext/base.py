...
class RedisAdapter:
    def __init__(self,
        ...
        self.redis_url = os.getenv("REDIS_URL", "redis://127.0.0.1:6379") \
            if redis_url is None else redis_url
        self.redis_threshold = float(os.getenv("REDIS_THRESHOLD", 0.05)) \
            if redis_threshold is None else redis_threshold
        self.redis_ttl = int(os.getenv("REDIS_TTL", 3600)) \
            if redis_ttl is None else redis_ttl
    ...
    def save(self, meta_info: str, query: str = "", output: str = "", json_data: Optional[dict] = None):
        ...
        """
        self.logger.info(f"saving query")
    ...
    def get(self, meta_info: str, query: str = "") -> Optional[Dict[str, Any]]:
        ...
        self.logger.info(f"getting query")
