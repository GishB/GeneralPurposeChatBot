...
class BM25Reranker:
    ...
    def preprocess(self, text: str) -> List[str]:
        ...
        logger.debug(f"Finished preprocessing over text")
        return token_strings

    def fit(self, documents: List[str]):
        """Обучение модели на корпусе документов для текущего запроса."""
        logger.info(f"Fitting {len(documents)} documents")
    ...
    def rerank(self, query: str, top_k: int = 3) -> List[int]:
        ...
        logger.info(f"Reranking | top_k={top_k}")
        ...
        logger.info(f"Finished reranking | top_k={top_k}")
