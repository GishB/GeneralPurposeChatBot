...
async def welcome_user(bot: Bot, message: Message):
    ...
    await bot.send_message(chat_id=message.chat.id, text=formatted_message)


async def private_chat(
    bot: Bot, message: Message, client, prompt: str, collection_name: str
) -> Union[str, None]:
    """
    ...
    try:
        ...
        await bot.send_message(chat_id=message.chat.id, text=error_msg)
        return None
